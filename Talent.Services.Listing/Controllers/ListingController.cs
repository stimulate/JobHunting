    ...
    public class ListingController : Controller
    {
        ...
        public async Task<IActionResult> GetEmployerJobs(string employerId=null)
        {
            try
            {
                ...
                var myJobs = (await _jobService.GetEmployerJobsAsync(employerId)).Select(x => new { x.Id, x.Title, x.EmployerID, x.Summary, x.JobDetails.Location, x.Status, noOfSuggestions=x.TalentSuggestions!=null && x.TalentSuggestions.Count!=0 ?x.TalentSuggestions.Count:0 });
        ...
        [HttpGet("getData")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "employer, recruiter")]
        public async Task<IActionResult> GetData(string employerId = null)
        {
            try
            {
                employerId = employerId == null ? _userAppContext.CurrentUserId : employerId;
                var myJobs = (await _jobService.GetEmployerJobsAsync(employerId)).Select(x => new { x.Id, x.Title, x.Summary,x.Description,x.ApplicantDetails, x.JobDetails, x.JobDetails.Location, x.CreatedOn, Expiry = x.ExpiryDate < DateTime.UtcNow && x.ExpiryDate != DateTime.MinValue, x.Status, noOfSuggestions = x.TalentSuggestions != null && x.TalentSuggestions.Count != 0 ? x.TalentSuggestions.Count : 0 });
                return Json(new { Success = true, MyJobs = myJobs });
            }
            catch
            {
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "employer, recruiter")]
        public async Task<IActionResult> GetSortedEmployerJobs(int activePage = 1, string sortbyDate = "asc", bool showActive = true, bool showClosed = false, bool showDraft = true, bool showExpired=true, bool showUnexpired=false, string employerId = null, int limit = 6)
        {
            try
            {
                employerId = employerId == null ? _userAppContext.CurrentUserId : employerId;
        ...
        public async Task<IActionResult> CloseJob(string id)
        {
            try
            {
                ...
                //}
                //else
                //    return Json(new { Success = false, Message = "You are not authorised to up date this job" });
            }
            catch (Exception e)
            {
